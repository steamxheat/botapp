...
# Добавление воркеров по умолчанию
def add_default_workers():
    conn = sqlite3.connect('gift_monitor.db')
    cursor = conn.cursor()
    
    # Добавляем админа и тестовых воркеров
    workers = [
        ("6038457276", "KA_RL_WOrk"),  # Админ
        ("123456789", "Worker_1"),     # Пример воркера
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if worker:
        # Воркер - показываем меню воркера
        keyboard = [
        ...
        # Обычный пользователь (мамонт) - показываем подарок с Web App
        await update.message.reply_text(
...
# Обработка старта через callback
...
# Показ подарка мамонту (альтернативный вариант без Web App)
